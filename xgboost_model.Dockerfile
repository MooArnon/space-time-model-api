...
COPY requirements.txt xgboost_requirements_uninstall.txt ${LAMBDA_TASK_ROOT}

# Install numpy, scipy, and scikit-learn
RUN pip install --no-cache-dir --upgrade pip setuptools && \
    pip install --no-cache-dir -r requirements.txt --timeout 600 && \
    pip uninstall -y -r xgboost_requirements_uninstall.txt && \
    rm requirements.txt xgboost_requirements_uninstall.txt
