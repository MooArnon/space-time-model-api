...
COPY requirements_deep.txt deep_requirements_uninstall.txt /

# Install numpy, scipy, and scikit-learn
RUN pip install --no-cache-dir --upgrade pip setuptools && \
    pip install --no-cache-dir -r requirements_deep.txt --timeout 600 && \
    pip uninstall -y -r deep_requirements_uninstall.txt && \
    rm requirements_deep.txt deep_requirements_uninstall.txt
